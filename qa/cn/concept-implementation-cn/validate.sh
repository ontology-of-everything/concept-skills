...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/cn/concept-implementation-cn" && pwd)"
...
check_skill_layout() {
  ...
  rg -q '^name: concept-implementation-cn$' "$SKILL_DIR/SKILL.md" || fail "frontmatter name mismatch"
  rg -q '\$concept-implementation-cn' "$SKILL_DIR/agents/openai.yaml" || fail "default prompt must name the skill"
...
printf 'OK: concept-implementation-cn validation passed\n'
