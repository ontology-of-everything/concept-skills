# concept-guardrails-cn Changelog
...
- 模式名去掉 `wyx:` 前缀：`$concept-guardrails-cn <audit|concept|drift|pipeline|sync|map> [路径或描述]`；
...
- Align Cursor with Codex: skill stays off until `/concept-guardrails-cn` or
  `$concept-guardrails-cn` (Codex already had `allow_implicit_invocation: false`).
...
- 技能改名：`wyx-zh-cn` → `concept-guardrails-cn`，与 `concept-design` /
  `concept-prd` / `concept-implementation` / `concept-audit` 共用 concept-*
  前缀；显式调用改为 `$concept-guardrails-cn`
- `runtime/.claude-plugin/plugin.json` 的插件标识同步改为 `concept-guardrails-cn`
