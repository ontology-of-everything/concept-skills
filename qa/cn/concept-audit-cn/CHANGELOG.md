...
- Default off: run only on explicit `$concept-audit-cn` / `/concept-audit-cn`. Codex
...
- Rename the installable skill from `jackson-concept-audit-cn` to
  `concept-audit-cn` (concept-* family prefix); companion references updated.
...
- Initial Jackson concept-audit-cn skill: read-only five-dimension check of an
